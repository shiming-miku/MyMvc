...
@Component
public class AccessTokenHuoQu {
	
	private static final Logger logger=LoggerFactory.getLogger(AccessTokenHuoQu.class);
	@Autowired
	private AccessTokenDao accessTokenDao;

	@Scheduled(cron="0 0 0/2 * * ?")
	public void accesstokenget() {
		logger.info("获取access_token定时任务开始执行开始");
		...
		try {
			...
			try {
				httpclient.close();
				logger.info("获取access_token定时任务开始执行结束");
			} catch (IOException e) {
				e.printStackTrace();
			}
		}
	...
	public static void main(String[] args) {
		AccessTokenHuoQu accesstoken=new AccessTokenHuoQu();
